...
# read in data
dat <- read.csv("data/new_datA.csv", sep = ";")
...
# correlation test between Height and delta13C
 ...
 # Others
 dat$Others <- dat$Pyr + dat$Org
 dat$non_Others <- dat$non_Pyr + dat$non_Org
 # remove columns
 dat <- dat[, !names(dat) %in% c("Pyr", "Org")]
 dat <- dat[, !names(dat) %in% c("non_Pyr", "non_Org")]
 
 # Non_Identified components
 dat$Non_Identified <- dat$non_Identified + dat$Tas
 dat$Identified <- dat$Identified + dat$non_Tas
 # remove columns
 dat <- dat[, !names(dat) %in% c("non_Identified", "Tas")]
 dat <- dat[, !names(dat) %in% c( "non_Tas")]
...
# save pdf
pdf("output/LMA_trends.pdf", width = 10, height = 10)
...
## part 2
pdf("output/LMA_trends2.pdf", width = 10, height = 10)
...
# plots
visreg(mod1,scale="response",xvar="height",ylab=expression("proportion of corals"),xlim=c(1,15), rug=F,  xlab=expression(bold("height [m]")))
visreg(mod2,scale="response",xvar="height",ylab=expression("proportion of sponges"),xlim=c(1,15), rug=F,  xlab=expression(bold("height [m]")))
visreg(mod3,scale="response",xvar="height",ylab=expression("proportion of bryozoans"),xlim=c(1,15), rug=F,  xlab=expression(bold("height [m]")))
visreg(mod4,scale="response",xvar="height",ylab=expression("proportion of microbes"),xlim=c(1,15), rug=F,  xlab=expression(bold("height [m]")))
visreg(mod12,scale="response",xvar="height",ylab=expression("proportion of micrite"),xlim=c(1,15), rug=F,  xlab=expression(bold("height [m]")))
visreg(mod13,scale="response",xvar="height",ylab=expression("proportion of sparite"),xlim=c(1,15), rug=F,  xlab=expression(bold("height [m]")))
